...
public class UnbanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!staff.hasPermission("naturalbans.unban")) {
        ...
        if (args[args.length-1].equals("-s")) {
            reason = getReasonUtil.getReason(args, 1, 1);
            staff.sendMessage(FileManager.getMessage("unban_success_silent")
                    .replace("{target}", target.getName())
                    .replace("{player}", staff.getName())
            );
        } else {
            reason = getReasonUtil.getReason(args, 1, 0);
            Bukkit.getServer().broadcastMessage(FileManager.getMessage("unban_success_loud")
                    .replace("{target}", target.getName())
                    .replace("{player}", staff.getName())
            );
        }

        PunishmentManager.unBan(target.getUniqueId(), reason, staff.getUniqueId());
