...
@Async
public class UnbanCommand implements CommandService {
    ...
    public void _def(@Context Player player) {
        if (!player.hasPermission("naturalbans.unban")) {
    ...
    public void unban(
        ...
        if (silent) {
            player.sendMessage(fileManager.getMessage("unban_success_silent")
