...
@Async
public class UnmuteCommand implements CommandService {
    ...
    public void _def(@Context Player player) {
        if (!player.hasPermission("naturalbans.unmute")) {
    ...
    public void unmute(
        ...
        if (silent) {
            player.sendMessage(fileManager.getMessage("unmute_success_silent")
